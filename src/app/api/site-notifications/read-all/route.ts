...
import { markAllRecentSiteNotificationsReadForUser } from "@/lib/site-notifications";
...
export async function POST() {
  try {
    ...
    const result = await markAllRecentSiteNotificationsReadForUser(session.user.id);
    if (!result.ok) {
